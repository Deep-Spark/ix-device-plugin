...
func (p *bestEffortPolicy) Allocate(arg PolicyArgs) []string {
	bestArg, ok := (arg).(BestPolicyArgs)
	if !ok {
		return []string{}
	}
	available := bestArg.Available
	required := bestArg.Required
	size := bestArg.Size
	var ret []string
	if size <= 0 {
		return []string{}
	...
	klog.Infof("best score:%v\n", bestScore)

	// Return the highest scoring GPU set.
	for _, dev := range bestSet {
...
func calculateGPUPairScore(gpu0 *Device, gpu1 *Device) int {
	...
	if len(gpu0.Links[gpu1.UUID]) != len(gpu1.Links[gpu0.UUID]) {
	...
	for _, link := range gpu0.Links[gpu1.UUID] {
		switch link.Type {
